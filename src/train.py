...
def parse_args():
    ...
    # data
    parser.add_argument(
        "--datadir", type=str, help="Data dir"
    )
    parser.add_argument(
        "--labeldir",
        type=str,
    ...
    # training
    parser.add_argument(
        "--epoch", type=int, default=100, help="Epoch to run [default: 1000]"
    )
    parser.add_argument(
        "--batch_size",
        type=int,
...
def val_test(model, dataloader, opt) -> dict:
    ...
    for j, (data, label) in tqdm(
        enumerate(dataloader), total=len(dataloader), desc=f"Vali"
    ):
...
def train(opt):
    ...
    for i in range(1, opt.epoch):
        ...
        if i <= opt.warmup_epoch:
            current_lr = opt.warmup_lr_init + (
                opt.warmup_lr_end - opt.warmup_lr_init
            ) * (i / opt.warmup_epoch)
        ...
        # Training
        for j, (data, label) in tqdm(
            enumerate(train_loader), total=len(train_loader), desc=f"Epoch {i} train"
        ):
            data = data.cuda(opt.gpu)
            label = label.cuda(opt.gpu)
    ...
    with open(os.path.join(savepath, "data.txt"), "w") as data_file:
        ...
        for k in range(len(train_metric["outputs"])):
            data_file.write(
                f"{train_metric['outputs'][k]:.4f},{train_metric['labels'][k]:.4f} "
            )
            if k % 10 == 0 and k != 0:
                data_file.write("\n")
        data_file.write("\n")
        ...
        for k in range(len(val_metric["outputs"])):
            data_file.write(
                f"{val_metric['outputs'][k]:.4f},{val_metric['labels'][k]:.4f} "
            )
            if k % 10 == 0 and k != 0:
                data_file.write("\n")
        data_file.write("\n")
