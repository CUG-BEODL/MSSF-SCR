...
if __name__ == '__main__':
    ...
    # Example for test
    opt = parse_args()
    # Tiff file folder (recommend use opt.tif_patch's folder)
    opt.img_path = '/test'
    
    # 建议先将要测试的影像裁剪为固定大小（patch的整数倍），并把数据按输入通道顺序排列拼接成一个文件
    # reshapeAllTiff 函数可以实现这个功能（见test_patchprocess.py）
    opt.tif_patch = 'forTestUse.tif'
